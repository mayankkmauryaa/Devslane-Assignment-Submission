    ...
    <div className='class="bg-[#F4F5F6]"'>
      ...
      <div className="flex flex-col sm:flex-row mx-[20px] gap-4 px-8 py-4 bg-white sm:mx-[120px] mb-[40px] mt-[10px]">
        ...
        <div className="ml-8 space-y-4">
          ...
          <input onChange={handleCount} value={count} type="number" placeholder='1' className='border-[1.5px] w-12 mr-1 pl-1 border-gray-400' />
          <button onClick={hanldeAddToCart} className="bg-[#FF5151] border-[1px] border-[#FF5151] text-white px-4 py-1 rounded text-sm">
            ADD TO CART
          </button>
        </div>
      </div>
      <div className='flex justify-between items-center px-6 pb-6 sm:px-32' >
        <div className='flex items-center gap-2 text-white px-2 py-1 rounded bg-orange-500'>
          <FaAngleDoubleLeft />
          <Link className='text-lg pb-1' to={'/details/' + (Id - 1)} >Prev</Link>
        </div>
        <div className='flex items-center gap-2 text-white px-2 py-1 rounded bg-orange-500'>
          <Link className='text-lg pb-1' to={'/details/' + (Id + 1)} >Next</Link>
          <FaAngleDoubleRight />
        </div>
      </div>
    </div>
